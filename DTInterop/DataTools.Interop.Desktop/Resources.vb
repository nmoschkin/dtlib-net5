...
#End Region
    ...
    (phIconBits As IntPtr,
     ...
     uFlags As Integer) As IntPtr
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function LoadStringResource(fileName As String, Optional uFlags As LoadLibraryExFlags = LoadLibraryExFlags.LOAD_LIBRARY_AS_DATAFILE Or LoadLibraryExFlags.LOAD_LIBRARY_AS_IMAGE_RESOURCE, Optional ByVal parseResName As Boolean = False, Optional maxBuffer As Integer = 4096) As String
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function LoadStringResource(fileName As String, resId As Integer, Optional uFlags As LoadLibraryExFlags = LoadLibraryExFlags.LOAD_LIBRARY_AS_DATAFILE Or LoadLibraryExFlags.LOAD_LIBRARY_AS_IMAGE_RESOURCE, Optional ByVal parseResName As Boolean = False, Optional maxBuffer As Integer = 4096) As String
            Dim hmod As IntPtr
            ...
            Try
                hmod = LoadLibraryEx(fileName, 0, uFlags)
            Catch ex As Exception
                Return Nothing
            End Try
        ...
        Public Function LoadAllLibraryIcons(fileName As String, desiredSize As StandardIcons, Optional uFlags As LoadLibraryExFlags = LoadLibraryExFlags.LOAD_LIBRARY_AS_DATAFILE Or LoadLibraryExFlags.LOAD_LIBRARY_AS_IMAGE_RESOURCE) As Collections.ObjectModel.ObservableCollection(Of LibraryIcon)
            ...
            Dim enumres As ResCol = EnumResources(hmod, RT_GROUP_ICON)
        ...
        Private Function _internalLoadLibraryIcon(fileName As String, iIcon As Integer, resIcon As String, desiredSize As StandardIcons, uFlags As LoadLibraryExFlags, enumRes As ResCol, parseIconIndex As Boolean, hMod As IntPtr) As Icon
            ...
            If resIcon IsNot Nothing Then
                ...
                If resIcon.Chars(0) = "#"c AndAlso IsNumber(resIcon.Substring(1)) Then
        ...
        hdata As IntPtr
            ...
            If enumRes Is Nothing Then enumRes = EnumResources(hMod, RT_GROUP_ICON)

            '' no RT_GROUP_ICONs, they must want an actual ICON.
            If enumRes Is Nothing Then
                enumRes = EnumResources(hMod, RT_ICON)
                    ...
                    For Each n In enumRes
                        ...
                        ElseIf Not IsIntResource(n) Then
                            If n.CharAt(0) = "#"c Then
                                ...
                                Dim s As String = n.GrabString(1)
                                Dim i As Integer
                ...
                idata = Marshal.PtrToStructure(hdata, GetType(BITMAPINFOHEADER))
                ...
                icn = Icon.FromHandle(hicon).Clone
        ...
        ''' <remarks></remarks>
        Public Function GetLibraryIconCount(Optional ByVal library As String = "%systemroot%\system32\shell32.dll") As Integer
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetLibraryBitmap(ByVal iBmp As Integer, ByVal iSize As Size, Optional ByVal uFlags As Integer = LR_DEFAULTCOLOR + LR_CREATEDIBSECTION, Optional ByVal library As String = "%systemroot%\system32\shell32.dll") As Bitmap
        ...
        fFlags As Integer
            ...
            i = SHGetItemInfo(lpItemID, 0&, lpInfo, Len(lpInfo), fFlags + SHGFI_PIDL)

            If (i <> 0&) Then

                If (hIml) <> Nothing Then
                    hIml = i
                End If
        ...
        ''' <remarks></remarks>
        Public Function GetFileIconIndex(ByVal lpFilename As String, Optional ByVal shil As SystemIconSizes = SystemIconSizes.ExtraLarge) As Integer
            Dim lpInfo As New SHFILEINFO
            Dim iFlags As Integer = 0
        ...
        ''' <remarks></remarks>
        Public Function GetSystemImageList(ByVal shil As SystemIconSizes) As IntPtr
            Dim lpInfo As New SHFILEINFO,
        i As IntPtr
            Dim riid As New Guid("46EB5926-582E-4017-9FDF-E8998DAA0950")
            'iFlags = SHGFI_ICON
        ...
        ''' <remarks></remarks>
        Public Function GetFileIconFromIndex(ByVal index As Integer, Optional ByVal shil As SystemIconSizes = SystemIconSizes.ExtraLarge) As System.Drawing.Icon
            Dim lpInfo As New SHFILEINFO,
        i As IntPtr
            Dim icn As Icon
            Dim riid As New Guid("46EB5926-582E-4017-9FDF-E8998DAA0950")
        ...
        ''' <returns>A System.Drawing.Icon object.</returns>
        ''' <remarks></remarks>
        Public Function GetItemIcon(ByVal lpItemID As IntPtr, Optional ByVal shil As SystemIconSizes = SystemIconSizes.ExtraLarge) As System.Drawing.Icon
            Dim lpInfo As New SHFILEINFO,
        i As IntPtr
            Dim icn As Icon
            Dim riid As New Guid("46EB5926-582E-4017-9FDF-E8998DAA0950")
        ...
        c As Integer
            ...
            Dim stp As Integer = bm.Stride / bm.Width
        ...
        Public Function MakeWPFImage(img As Icon, Optional ByRef bitPtr As IntPtr = Nothing, Optional dpiX As Double = 96.0#, Optional dpiY As Double = 96.0#, Optional createOnApplicationThread As Boolean = True) As System.Windows.Media.Imaging.BitmapSource
            Return MakeWPFImage(IconToTransparentBitmap(img), bitPtr, createOnApplicationThread)
        End Function
        ...
        Public Function MakeWPFImage(img As System.Drawing.Bitmap, Optional ByRef bitPtr As IntPtr = Nothing, Optional dpiX As Double = 96.0#, Optional dpiY As Double = 96.0#, Optional createOnApplicationThread As Boolean = True) As System.Windows.Media.Imaging.BitmapSource
            ...
            Dim BytesPerRow As Integer = (((img.Width * 32 + 31) And (Not 31)) / 8)
        ...
        Public Function MakeBitmapFromWPF(source As System.Windows.Media.Imaging.BitmapSource) As System.Drawing.Bitmap
            ...
            Sub()
                ...
                source.CopyPixels(System.Windows.Int32Rect.Empty, mm.Handle, mm.Length, bmp.Width * 4)
        ...
        Public Function MakeDIBSection(img As Bitmap, Optional ByRef bitPtr As IntPtr = Nothing) As IntPtr
            ...
            Dim BytesPerRow As Integer = (((img.Width * wBitsPerPixel + 31) And (Not 31L)) / 8L)
            Dim size As Integer = img.Height * BytesPerRow
            Dim bmpInfo As BITMAPINFO
            Dim mm As New MemPtr
            Dim bmpSizeOf As Integer = Marshal.SizeOf(bmpInfo)
            mm.SetLength(bmpSizeOf + size)
            ...
            pbmih.biXPelsPerMeter = 24.5 * 1000 ' pixels per meter! And these values MUST be correct if you want to pass a DIB to a native menu.
            pbmih.biYPelsPerMeter = 24.5 * 1000 ' pixels per meter!
            pbmih.biClrUsed = 0
            ...
            Dim hPreviewBitmap As IntPtr = CreateDIBSection(IntPtr.Zero, mm.Handle, DIB_RGB_COLORS, pPixels, IntPtr.Zero, 0)
            ...
            For curY As Integer = 0 To img.Height - 1
                MemCpy(pCurrDest, pCurrSource, BytesPerRow)
        ...
        c As Integer
            ...
            For i = 0 To c
                ...
                b(i) = ColorMath.HSVToColor(hsv2).ToArgb
            Next
