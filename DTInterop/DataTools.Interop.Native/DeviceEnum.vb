...
Namespace Native
    ...
    Friend Module DevEnumApi
                    ...
                    ElseIf comp(i).DeviceClass = DeviceClassEnum.Volume Then
                        ...
                        vl.PDOName.ToUpper.Trim = comp(i).PDOName.ToUpper.Trim Then
                                ...
                                If vl.VolumePaths IsNot Nothing AndAlso vl.VolumePaths.Count > 0 Then
                                    vl.DeviceIcon = MakeWPFImage(GetFileIcon(vl.VolumePaths(0), SHIL_EXTRALARGE))
                                End If
        ...
        Public Function _internalGetComputer(Optional noLink As Boolean = False, Optional classOnly As Guid = Nothing) As DeviceInfo()
            ...
            Dim lIcon As New CollectionOfKeyValue(Of Guid, System.Drawing.Icon)
            ...
            If classOnly <> Guid.Empty Then
                hDev = SetupDiGetClassDevs(classOnly, IntPtr.Zero, IntPtr.Zero, DIGCF_PRESENT)

            Else
                hDev = SetupDiGetClassDevs(Nothing, IntPtr.Zero, IntPtr.Zero, DIGCF_ALLCLASSES Or DIGCF_PRESENT Or DIGCF_DEVICEINTERFACE)
            ...
            devInterface.cbSize = Marshal.SizeOf(devInterface)

            While SetupDiEnumDeviceInfo(hDev, c, devInfo)

                Try
                    ReDim Preserve devOut(c)

                    devOut(c) = _internalPopulateDeviceInfo(Of DeviceInfo)(hDev, Nothing, devInfo, devInterface, mm)
                    SetupDiEnumDeviceInterfaces(hDev, IntPtr.Zero, devOut(c).DeviceClassGuid, c, devInterface)
                    ...
                    If Not lIcon.ContainsKey(devOut(c).DeviceClassGuid) Then
                        ...
                        If hicon <> IntPtr.Zero Then
                            icn = Drawing.Icon.FromHandle(hicon).Clone
                            DestroyIcon(hicon)
                        End If

                        lIcon.AddNew(devOut(c).DeviceClassGuid, icn)
                        devOut(c).DeviceClassIcon = icn
                    Else
                        devOut(c).DeviceClassIcon = lIcon.ItemByKey(devOut(c).DeviceClassGuid)
                    End If
        ...
        Public Function _internalGetProperty(info As DeviceInfo, prop As DEVPROPKEY, type As DevPropTypes, Optional flags As ClassDevFlags = ClassDevFlags.Present Or ClassDevFlags.DeviceInterface, Optional useClassId As Boolean = False) As Object
            ...
            SetupDiDestroyDeviceInfoList(hDev)
            _internalGetProperty = _internalDevPropToObject(type, mm, c)
            mm.Free()
        ...
        Public Function _internalEnumerateDevices(Of T As {DeviceInfo, New})(ClassId As Guid, Optional flags As ClassDevFlags = ClassDevFlags.Present Or ClassDevFlags.DeviceInterface) As T()
            ...
            If flags And ClassDevFlags.DeviceInterface Then

                While SetupDiEnumDeviceInterfaces(hDev, IntPtr.Zero, ClassId, c, devInterface)
                    ReDim Preserve devOut(c)

                    SetupDiEnumDeviceInfo(hDev, c, devInfo)
        ...
        ''' <remarks></remarks>
        Public Function _internalEnumDisks(Optional ByVal DiskClass As Guid = Nothing) As DiskDeviceInfo()
            ...
            Try
                ...
                For Each inf In info
                    ...
                    inf.Capabilities = _internalGetProperty(inf, DEVPKEY_Device_Capabilities, DevPropTypes.Int32)
                    If inf.Capabilities = 0 Then
                        inf.Capabilities = _internalGetProperty(inf, DEVPKEY_Device_Capabilities, DevPropTypes.Int32, , True)
                    End If
                    ...
                    If (DiskClass = GUID_DEVINTERFACE_VOLUME) OrElse (DiskClass = GUID_DEVCLASS_VOLUME) Then
                        ...
                        If disk <> INVALID_HANDLE_VALUE Then
                            ...
                            If bytesReturned > 0 Then
                                diskNumber = Marshal.PtrToStructure(hHeap.handle, GetType(STORAGE_DEVICE_NUMBER))
                                inf.PhysicalDevice = diskNumber.DeviceNumber

                                If diskNumber.PartitionNumber < 1024 Then
                                    inf.PartitionNumber = diskNumber.PartitionNumber
                                End If
                            End If
                    ...
                    If inf.FriendlyName = "Microsoft Virtual Disk" Then
                        ...
                        mm.IntegerAt(0) = STORAGE_DEPENDENCY_INFO_VERSION.STORAGE_DEPENDENCY_INFO_VERSION_2
                        mm.IntegerAt(1) = 1

                        mm.IsString = True
                        r = GetStorageDependencyInformation(disk, GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_HOST_VOLUMES + GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_DISK_HANDLE, mm.Length, mm, su)
                        If su <> sdic Then
                            mm.Length = su
                            r = GetStorageDependencyInformation(disk, GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_HOST_VOLUMES + GET_STORAGE_DEPENDENCY_FLAG.GET_STORAGE_DEPENDENCY_FLAG_DISK_HANDLE, mm.Length, mm, su)
                        End If
                        ...
                        Else
                            sdi.NumberEntries = mm.UIntegerAt(1)
                            ReDim inf.BackingStore(sdi.NumberEntries - 1)

                            For d = 0 To sdi.NumberEntries - 1
                                sdi.Version2Entries = Marshal.PtrToStructure(mm.DangerousGetHandle + (sdic * d) + 8, GetType(STORAGE_DEPENDENCY_INFO_TYPE_2))
                                inf.BackingStore(d) = (Path.GetFullPath(sdi.Version2Entries.DependentVolumeRelativePath))
                            Next
                        End If
            ...
            If infoOut.DeviceIcon Is Nothing Then
                ...
                If infoOut.ContainerId <> Guid.Empty And noStaging = False Then
                    ...
                    If stagingClasses.Contains(infoOut.DeviceClass) Then
                        ...
                        If mm <> IntPtr.Zero Then
                            '' Get a WPFImage
                            infoOut.DeviceIcon = MakeWPFImage(GetItemIcon(mm, SHIL_EXTRALARGE))
                            mm.Free()
                ...
                If infoOut.DeviceIcon Is Nothing Then
                    If SetupDiLoadDeviceIcon(hDev, devInfo, 48, 48, 0, hIcon) Then
                        ...
                        Dim icn2 As System.Drawing.Icon = icn.Clone
        ...
        Public Function _internalPopulateDeviceInfo(Of T As {DeviceInfo, New})(hDev As IntPtr,
            ...
            Try
                ...
                If cbSize > 0 Then
                    ...
                    mm.Length = cbSize
                    mm.IntegerAt(0) = If(IntPtr.Size = 4, 6, 8)
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then
                        devOut.DeviceClassGuid = New Guid(mm.GrabBytes(0, 16))
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then
                        devOut.DeviceInterfaceClassGuid = New Guid(mm.GrabBytes(0, 16))
                        devOut.DeviceInterfaceClass = GetDevInterfaceClassEnumFromGuid(devOut.DeviceInterfaceClassGuid)
                    End If
                End If
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.InstallDate = _internalDevPropToObject(DevPropTypes.FileTime, mm, cbSize)
                End If
                '' Characteristics
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.Characteristics = mm.IntegerAt(0)
                End If
                '' Removal Policy
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.RemovalPolicy = mm.IntegerAt(0)
                End If
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.BusType = GuidToBusType(New Guid(mm.GrabBytes(0, 16)))
                End If
                '' ContainerId
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.ContainerId = New Guid(mm.GrabBytes(0, 16))
                End If
                ...
                If cbSize > 0 Then
                    ...
                    pt = mm.handle
                    If cbSize > 0 Then devOut.Children = pt.GrabStringArray(0)
                End If
                '' HardwareIds
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.HardwareIds = pt.GrabStringArray(0)
                End If
                '' LocationPaths
                ...
                If cbSize > 0 Then
                    ...
                    pt = mm.handle
                    If cbSize > 0 Then devOut.LocationPaths = pt.GrabStringArray(0)
                End If
                '' Parent Device
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then
                        ...
                        devOut.PhysicalPath = mm.GrabBytes(0, cbSize)
                    End If
                ...
                If cbSize > 0 Then
                    ...
                    If cbSize > 0 Then devOut.UINumber = mm.IntegerAt(0)
                End If
            ...
            Catch ex As Exception
                ...
                sb.AppendLine(FormatLastError(Marshal.GetLastWin32Error))

                MsgBox(ex.Message & " : See " & dumpFile & " for more clues." & vbCrLf & FormatLastError(Marshal.GetLastWin32Error))
                File.AppendAllText(dumpFile, sb.ToString)
        ...
        Public Function _internalDevPropToObject(type As DevPropTypes, data As IntPtr, Optional length As Integer = 0) As Object
            ...
            Select Case type
                ...
                Case DevPropTypes.Int32
                    Return mm.IntegerAt(0)

                Case DevPropTypes.UInt32
                    Return mm.UIntegerAt(0)
