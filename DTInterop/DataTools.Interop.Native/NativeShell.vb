...
#Region "Shell Helper Methods"
        ...
        <DllImport("shell32.dll", CharSet:=CharSet.Unicode, SetLastError:=True)>
        Friend Function SHCreateItemFromParsingName(<MarshalAs(UnmanagedType.LPWStr)> path As String, pbc As IntPtr, ByRef riid As Guid, <MarshalAs(UnmanagedType.[Interface])> ByRef shellItem As IShellItem2) As Integer
        End Function

        ' The following parameter is not used - binding context.
        <DllImport("shell32.dll", CharSet:=CharSet.Unicode, SetLastError:=True)>
        Friend Function SHCreateItemFromParsingName(<MarshalAs(UnmanagedType.LPWStr)> path As String, pbc As IntPtr, ByRef riid As Guid, <MarshalAs(UnmanagedType.[Interface])> ByRef shellItem As IShellItem) As Integer
        End Function

        <DllImport("shlwapi.dll", CharSet:=CharSet.Unicode, SetLastError:=True)>
        Friend Function PathParseIconLocation(<MarshalAs(UnmanagedType.LPWStr)> ByRef pszIconFile As String) As Integer
        End Function
...
#End Region
        ...
        Public Function EnumFileHandlers(fileExtension As String, Optional flush As Boolean = False) As IAssocHandler()
            ...
            If lAssoc.Count Then lAssoc.Clear()
...
#Region "File Association Handlers"
    ...
    Public Interface IAssocHandler
        ...
        <PreserveSig>
        <MethodImpl(MethodImplOptions.InternalCall, MethodCodeType:=MethodCodeType.Runtime)>
        Function MakeDefault(<MarshalAs(UnmanagedType.LPWStr), [In]> ByVal pszDescription As String) As HResult
